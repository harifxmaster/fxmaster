...
const App = () => {
  ...
  useEffect(() => {
    async function loadDefaultData(){
      await axios.get(Constants.BASE_URL+"API-FX-100-App",{headers:{
      ...
      await axios.get(Constants.BASE_URL+"API-FX-101-Title",{headers:{
        fx_key:Constants.SUBSCRIPTION_KEY
      }}).then(response=>{
      ...
      await axios.get(Constants.BASE_URL+"API-FX-140-Occupation",{headers:{
        fx_key:Constants.SUBSCRIPTION_KEY
      }}).then(response=>{
      ...
      await axios.get(Constants.BASE_URL+"API-FX-102-Country",{headers:{
        fx_key:Constants.SUBSCRIPTION_KEY
      }}).then(response=>{
      ...
      await axios.get(Constants.BASE_URL+"API-FX-103-Nationality",{headers:{
        fx_key:Constants.SUBSCRIPTION_KEY
      }}).then(response=>{
